define([
    ...
    /**
     * Bind events to add empty Dropdown Content when Dropdown added and reinitialize accordion when Dropdown content added
     */
    Preview.prototype.bindEvents = function bindEvents() {
        ...
        events.on('dropdown:dropAfter', function(args) {
            if (args.id === self.contentType.id && self.contentType.children().length === 0) {
                self.addDropdownContent();
            }
        });

        events.on('dropdown-content:renderAfter', args => {
     ...
     */
    Preview.prototype.addDropdownContent = function() {
        var self = this;
        createContentType(
            pageBuilderConfig.getContentTypeConfig('dropdown-content'),
            this.contentType,
    ...
    Preview.prototype.retrieveOptions = function() {
        ...
        options.add = new option({
            ...
            title: 'Add',
            action: self.addDropdownContent,
            classes: ['add-child'],
            sort: 10,
        });
        return options;
